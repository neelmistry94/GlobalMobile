...
//Rare exceptions to indicate a bug in the _hello code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_hello = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_hello]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.hello.module"
